...
#define Success2(v) (option_t){.success = true, .value = v.p};
...
static entry_t *find_previous_entry_for_key(entry_t *entry, elem_t key)
{
  ...
  if (k1.i > key.i && key.i > 0) // För att kunna hitta negativa värden i buckets.
  {
    return entry->next;
  }
  else
...
static int negative_key(elem_t key)
{
  int bucket = 0;
  if (key.i < 0)
  {
    bucket = mod(key.i) % No_Buckets;
  }
  else
  {
    bucket = key.i % No_Buckets;
  }
  return bucket;
}
